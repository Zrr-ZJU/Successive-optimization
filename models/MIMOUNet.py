...
class SCMF(nn.Module):
    def __init__(self, out_plane):
        ...
        self.main = nn.Sequential(
            ...
            BasicConv(out_plane // 4, out_plane // 2, kernel_size=1, stride=1, relu=True),
            BasicConv(out_plane // 2, out_plane // 2, kernel_size=3, stride=1, relu=True),
            BasicConv(out_plane // 2, out_plane-5, kernel_size=1, stride=1, relu=True)
        )
...
class MIMOUNetFov(nn.Module):
    def __init__(self, num_res=8):
        super(MIMOUNetFov, self).__init__()
        ...
        self.feat_extract = nn.ModuleList([
            ...
            BasicConv(base_channel*4, base_channel*2, kernel_size=4, relu=True, stride=2, transpose=True),
            BasicConv(base_channel*2, base_channel, kernel_size=4, relu=True, stride=2, transpose=True),
            BasicConv(base_channel, 5, kernel_size=3, relu=False, stride=1)
        ])
        ...
        self.ConvsOut = nn.ModuleList(
            [
                BasicConv(base_channel * 4, 5, kernel_size=3, relu=False, stride=1),
                BasicConv(base_channel * 2, 5, kernel_size=3, relu=False, stride=1),
            ]
        )
        ...
        self.FAM1 = FAM(base_channel * 4)
        self.SCM1 = SCMF(base_channel * 4)
        self.FAM2 = FAM(base_channel * 2)
        self.SCM2 = SCMF(base_channel * 2)
